...
export const list = query({
  ...
  handler: async (ctx, args) => {
    ...
    return tasks.filter(
      (t) => t.status !== "done" || (t.updatedAt ?? t._creationTime) >= oneDayAgo
    );
  },
});
...
export const listByAgent = internalQuery({
  ...
  handler: async (ctx, args) => {
    ...
    const reviewTasks = await ctx.db
      .query("tasks")
      .withIndex("by_harbor_status", (q) =>
        q.eq("harborId", args.harborId).eq("status", "review")
      )
      .collect();

    const reviewerTasks = reviewTasks.filter(
      (t) =>
        t.reviewerIds?.includes(agent._id) &&
...
export const internalCreateTask = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const assignee = await ctx.db
      ...
      .first();
    if (!assignee) throw new Error(`Assignee not found: ${args.assigneeSessionKey}`);

    // Resolve reviewers
    const reviewerIds: Array<typeof assignee._id> = [];
...
export const review = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    const hasChangesRequested = allReviewerIds.some(
      (id) => reviewStatuses[id] === "changes_requested"
    );
    const allApproved = allReviewerIds.every(
      (id) => reviewStatuses[id] === "approved"
    );
