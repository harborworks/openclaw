...
async function cognitoRequest(region, accessKeyId, secretAccessKey, target, body) {
...
export const invite = action({
    ...
    handler: async (ctx, args) => {
        ...
        const result = await cognitoRequest(region, accessKeyId, secretAccessKey, "AWSCognitoIdentityProviderService.AdminCreateUser", JSON.stringify({
...
export const deleteUser = action({
    ...
    handler: async (ctx, args) => {
        ...
        const callerOk = await ctx.runQuery(internal.admin.users.verifySuperAdmin, { cognitoSub: args.cognitoSub });
        if (!callerOk)
            throw new Error("Forbidden: superAdmin required");
        const target = await ctx.runQuery(internal.admin.users.getByIdInternal, { id: args.userId });
        if (!target)
        ...
        const { userPoolId, accessKeyId, secretAccessKey, region } = getAwsConfig();
        // Delete from Cognito
        await cognitoRequest(region, accessKeyId, secretAccessKey, "AWSCognitoIdentityProviderService.AdminDeleteUser", JSON.stringify({
            UserPoolId: userPoolId,
